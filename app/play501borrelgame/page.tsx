...
function Play501GameContent() {
  ...
  const [drinkPopupInfo, setDrinkPopupInfo] = useState<{
    player: Player;
    threshold: number;
    score: number;
    sips: number;
  ...
  useEffect(() => {
    ...
    const sipMultiplierParam = searchParams.get("sipMultiplier");
      ...
      try {
        ...
        if (!Array.isArray(parsedPlayers) || parsedPlayers.length === 0) {
          if (typeof window !== "undefined") {
            router.push("/speel501borrelmodus");
          }
          return;
        }
        ...
        // Parse difficulties per speler (optioneel)
        if (difficultiesParam) {
          try {
            const parsedDifficulties = JSON.parse(difficultiesParam) as Record<
              string,
              "easy" | "medium" | "hard" | "extreme"
            >;
            const diffMap = new Map<number, "easy" | "medium" | "hard" | "extreme">();
            parsedPlayers.forEach((player: Player) => {
              const raw = parsedDifficulties[String(player.id)];
              const value: "easy" | "medium" | "hard" | "extreme" =
                raw === "easy" || raw === "medium" || raw === "hard" || raw === "extreme"
                  ? raw
                  : "medium";
              diffMap.set(player.id, value);
            });
            setPlayerDifficulties(diffMap);
          } catch (err) {
            console.error("Error parsing difficulties:", err);
          }
        }

        // Parse sip multiplier (optioneel)
        if (sipMultiplierParam) {
          const parsedMultiplier = parseInt(sipMultiplierParam, 10);
          if (!Number.isNaN(parsedMultiplier) && parsedMultiplier > 0) {
            setSipMultiplier(parsedMultiplier);
          }
        }
          ...
          <button
            onClick={() => {
              if (startMethod) {
                ...
                // Als we in het menu zitten, ga naar speel501borrelmodus
                router.push("/speel501borrelmodus");
              }
            }}
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0A294F] pt-4 pb-6 px-4">
      ...
      {/* Drink Popup - borrelmodus */}
      {
        showDrinkPopup && drinkPopupInfo && (
          <>
            {/* Overlay */}
            <div
              className="fixed inset-0 bg-[#0A294F] bg-opacity-40 backdrop-blur-sm z-40 transition-opacity duration-300"
              onClick={() => {
                // voorkom sluiten via overlay
              }}
            />
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <div
                className="bg-[#E8F0FF] rounded-2xl p-6 shadow-2xl w-full max-w-md border-4 border-[#0A294F]"
                style={{
                  ...
                  {drinkPopupInfo.is180 ? (
                    ...
                    <>
                      <p className="text-[#0A294F] text-sm mb-2">
                        ...
                        punten, minder dan{" "}
                        <span className="font-semibold">{drinkPopupInfo.threshold}</span>.
                      </p>
